...
def normalize_traps(array_image):
    ...
    for index in range(0, Y):
        for sub_index in range(0, X):
            if array_image[index][sub_index] != 0:
                occurrence = occurrence + 1
    print(" the total number of traps :", str(occurrence))
...
iteration = 45  # iterations for IFTA in total
iteration_for_GSA = 30  # iterations for IFTA without feedback
color = ["r", "g", "b", "y", "c", "k", "m"]
...
# image array, which returns a triangular arrays after np.fft.fftshift
#array_image = gen_triangle_array(trap_number, center_spacing, trap_spacing_x, trap_spacing_y, number_of_row, number_of_col)
array_image = arbitrary()
...
for iteration_s in range(0, iteration):
    ...
    if iteration_s <= iteration_for_GSA:
        ...
        avg_buffer.append(average_intensity)
    ...
    #"Start running the camera feedback"
        ...
        while boolean:
            ...
            try:
                response = input("is the trap intensity satisfying yes/no = 0/1 :")
        ...
        while len(intensity_buffer)< total_nums_traps(array_image_chopped_cp) :    # if the number of trap image is not enough, the code keeps runnning
            # print("the intensity buffer length :" ,len(intensity_buffer) )
            # capturing the image from camera
            if n < 17:
                if m<25:    # this line can be potentially to read out the trap coodinate
                    if m == 0:
                        x_corr = 0
                    ...
                    # if m == 0 :
                    #     plt.imshow(CMOS_traps, cmap=color_map[color_index])
                    #     plt.title("valid trap")
                    #     plt.show()
                    #     print("Valid trap:", "up:", up_value, "down:", down_value, "left:", left_value, "right:",
                    #           right_value, "central:", central_value, "The LR difference:" ,right_value - left_value, "The UD difference", up_value - down_value)
                    #
                    #     print("n and m :", str(n), str(m), "coord", trap_x,  trap_y)
                    ...
                    if True:
                        ...
                        while np.abs(right_value - left_value) > 0.15or np.abs(up_value - down_value) >0.15: # which means the trap off centered
                            ...
                            while right_value - left_value > 0.15: # fix the horrizontal line
                                ...
                                t_max = np.amax(Modified_CMOS_image)
                ...
                else:  # when the scanning hits the boundary, then move to a new row
                ...
                plt.imshow(cmap=color_map[color_index])
