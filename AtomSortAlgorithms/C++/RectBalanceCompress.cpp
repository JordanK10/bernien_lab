...
RectBalanceCompress::RectBalanceCompress(int Sites, float Prob)
{
    Array = MakeRectArray(Sites,Prob);
}

vector<vector<vector<int>>> RectBalanceCompress::Interpolate(vector<int> movefrom, vector<int> moveto, int center){
...
vector<vector<bool>> RectBalanceCompress::MakeRectArray(int Sites,float Prob){
    int i = 0;
    int j = 0;
    int Width;
    int Height;
...
tuple<int,int> RectBalanceCompress::ArrayDimensions(int sites, float prob){
...
vector<int> RectBalanceCompress::CenterOfMass(vector<vector<bool>> Array, int Height, int Width){
    int i = 0;
    int j = 0;
    float RowWeight = 0;
    float ColWeight = 0;
...
bool RectBalanceCompress::Toggle(bool a){
    bool b;
    if(a == false){
        b = true;
    }else{
...
vector<vector<vector<int>>> RectBalanceCompress::Bank(vector<vector<bool>> Array, int Range){
    vector<vector<vector<int>>> moves = {};
...
tuple<vector<bool>,vector<vector<int>>> RectBalanceCompress::CompressRow(vector<bool> row, int center, int suff){
    int dim = row.size();
    float x = suff;
    x/=2;
...
vector<vector<bool>> RectBalanceCompress::Move(vector<vector<bool>> Array, vector<int> pos1,vector<int> pos2){
...
vector<int> RectBalanceCompress::RowSum(vector<vector<bool>> Array, int H, int W){
    vector<int> RowTotal;
    int dummy = 0;
    int i = 0;
    int j = 0;
...
vector<vector<bool>> RectBalanceCompress::Transpose(vector<vector<bool>> Array, int dim){
    int i = 0;
    int j = 0;
...
bool RectBalanceCompress::Check(vector<vector<bool>> Array, vector<int> RowRange, vector<int> ColRange){
    int i = RowRange[0];
    int j = ColRange[0];
    bool check = true;
    while(i<RowRange[1]){
...
tuple<vector<vector<bool>>,vector<int>,vector<int>,vector<int>,vector<vector<vector<int>>>> RectBalanceCompress::Balance(vector<vector<bool>> Array, vector<int> Range, vector<int> COM, int Height,int Width, vector<int> RowTotal){
    vector<vector<vector<int>>> moves;
...
tuple<double, vector<vector<vector<int>>>,vector<int>,vector<vector<vector<int>>>,vector<vector<vector<int>>>, bool> RectBalanceCompress::BalanceCompress(vector<vector<bool>> Array){
...
return make_tuple(duration,moves2,rows,rowmoves,BankMoves,check);
}
